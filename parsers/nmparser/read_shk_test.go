...
func TestReadParseShkLines(t *testing.T) {
	...
	for _, tt := range tests {
		...
		assert.Equal(t, 4, len(shk.EtaBar), "Fail :"+tt.context)

		sd := ParseShkData(ed, tt.etaCount, tt.epsCount)
		assert.Equal(t, -0.00367026, sd[0].EtaBar[0], "Fail :"+tt.context)
		assert.Equal(t, 7.45166, sd[1].EbvVR[0], "Fail :"+tt.context)
		assert.Equal(t, 60.4504, sd[1].EbvVR[3], "Fail :"+tt.context)

		assert.Equal(t, tt.epsCount, len(sd[0].EpsSD), "Fail :"+tt.context)
		assert.Equal(t, tt.epsCount, len(sd[2].EpsSD), "Fail :"+tt.context)
		assert.Equal(t, tt.epsCount, len(sd[0].EpsVR), "Fail :"+tt.context)
		assert.Equal(t, tt.epsCount, len(sd[2].EpsVR), "Fail :"+tt.context)

		assert.Equal(t, tt.etaCount, len(sd[0].EtaBar), "Fail :"+tt.context)
		assert.Equal(t, tt.etaCount, len(sd[2].EtaSD), "Fail :"+tt.context)
